...
public class Base58 {
     ...
     * @return the base58-encoded string
     */
    public static String encodeChecked(int version, byte[] payload) throws Exception {
    ...
    private static byte[] hashTwice(byte[] data) throws Exception {
        MessageDigest h1 = MessageDigest.getInstance("SHA-256");
        h1.update(data);
        return MessageDigest.getInstance("SHA-256").digest(h1.digest());
    }

    private static byte[] hashTwice(byte[] data, int offset, int len) throws Exception {
        MessageDigest h1 = MessageDigest.getInstance("SHA-256");
        h1.update(data, offset, len);
        return MessageDigest.getInstance("SHA-256").digest(h1.digest());
    }
    ...
    public static byte[] decodeChecked(String input) throws Exception {
        ...
        byte[] h2 = hashTwice(data);
        byte[] actualChecksum = Arrays.copyOfRange(h2, 0, 4);
